...
lspectra$Frequency <- as.numeric(substr(lspectra$Freq, 0, 
                                        str_length(lspectra$Freq) -2))
...
Now, let's create a mean spectrogram with annotations that mark the frequency 
bands.
...
participants not included in the final analysis, and correct for multiple
comparisons by band using the BH (False Discovery) method.
...
We save the correlation data to a file (it will be used to print the topomaps 
with MNE) and then we display the full table of statistics:

```{r}

write_csv(Rdata, "correlations_channel_band_eyes_closed.csv", col_names = T)
...
And here is a visualization of corresponding distributions of _r_ and _p_ 
values. The dashed lines correspond to a significant threshold of _p_  < .05
on either a _r_ or a _p_-value scale. 

```{r}
ggplot(Rdata, aes(x=Band, y=r, col=Channel)) +
  geom_point() +
  stat_summary(fun.data = "mean_se", col="black", alpha=0.5,
               geom = "errorbar") +
  facet_wrap(~ material) +
...
ggplot(Rdata, aes(x=Band, y=p, col=Channel)) +
  geom_point() +
  stat_summary(fun.data = "mean_se", col="black", alpha=0.5, 
               geom = "errorbar") +
