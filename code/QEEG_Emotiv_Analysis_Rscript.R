...
analyze.logfile <- function(subject, session, sampling=128, window=2, sliding=0.75) {	
        ...
        file <- paste(subject, "_", session, ".csv", sep="")
...
analyze.folders <- function(session="pre", sampling=128) {
        for (d in dir()[file.info(dir())$isdir] ) {
                ...
                file <- paste(d, "_", session, ".csv", sep="")	
                print(c(d, session))
                setwd(d)
                if ( file.exists(file) ) {
